    ...
    <nav className="bg-white border-b border-slate-100 px-6 py-4 shadow-sm w-full sticky top-0 z-50 font-sans">
      ...
      {isOpen && (
        <div className="md:hidden mt-4 pb-4 border-t border-slate-100 pt-4 flex flex-col space-y-3 font-medium text-slate-600 animate-fadeIn">
          <Link to="/mainhome" onClick={() => setIsOpen(false)} className="hover:text-emerald-600 transition px-2 py-1">
            Home
          </Link>
          <Link to="/products" onClick={() => setIsOpen(false)} className="hover:text-emerald-600 transition px-2 py-1">
            Products
          </Link>
          <Link to="/dashboard" onClick={() => setIsOpen(false)} className="hover:text-emerald-600 transition px-2 py-1">
            Dashboard
          </Link>
          <Link to="/about" onClick={() => setIsOpen(false)} className="hover:text-emerald-600 transition px-2 py-1">
            About Us
          </Link>
          <Link to="/seller-profile" onClick={() => setIsOpen(false)} className="hover:text-indigo-400 transition px-2 py-1">
            Seller Profile
          </Link>
          <Link to="/chat" onClick={() => setIsOpen(false)} className="hover:text-indigo-400 transition px-2 py-1">
            Chat
          </Link>
